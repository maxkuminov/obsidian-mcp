...
@tools._tracked("probe_raises", ["value"], resource_class="light")
async def probe_raises(value: str = "x", exc: BaseException | None = None) -> str:
    raise exc if exc is not None else Boom("the body failed")


@tools._tracked("probe_ok", ["value"], resource_class="light")
async def probe_ok(value: str = "x") -> str:
    return f"ran:{value}"


@tools._tracked("probe_marks_then_raises", [], resource_class="light")
