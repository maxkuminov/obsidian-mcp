...
addressable three ways — exact heading text, the `Parent/Child`
path-style form, and the `#N` ordinal shown in the outline. The ordinal
is the only form that separates **duplicate sibling** headings, which
share every ancestor and so cannot be disambiguated by path; notes
generated by bulk extraction tend to be full of them.
