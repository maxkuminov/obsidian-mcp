# internal-transport-security Specification

## Purpose
How the server protects its internal hops (PostgreSQL and the embedding endpoint): database TLS is decided by `DATABASE_SSL_MODE` alone, strict modes verify every connection and refuse plaintext before and after connecting, the active embedding URL must be `https` or loopback unless plaintext is explicitly acknowledged, every embedding HTTP client comes from one factory, and each hop's transport is reported at startup. Created by the archived change `internal-transport-tls` (#184, #185 application half).
## Requirements
