...
- **Gate outcome (S4, 2026-09-22): PASS, the index ships.** Measured with `tests/integration/test_search_recall.py` on pgvector 0.8.6 / pg16, index built through `create_index_sql`, exact baseline a full-precision `vector` sequential scan:
  - filtered `semantic_search`: set recall **1.00** on all 60 cases (folder, tags, frontmatter, user scope × 5 queries × 3 rebuilds);
  - `find_related`: **1.00** on all 9 (3 hubs × 3 rebuilds).

  The same benchmark on the pre-change tree (`vector` index) also measured 1.00 on every case, so the change is recall-neutral on this corpus. Index size on a synthetic 17.5 k × 1024 corpus: 137 MB (`vector`) vs 46 MB (`halfvec`).
- **Implementation note (S4).** The query-side cast dimension is taken from the query vector's length rather than read from `settings.embedding_dimensions`. In production the two are equal: the lifespan's dimension check refuses to start otherwise, and a query of another length fails against the `vector(D)` column anyway. Reading the length keeps the cast in agreement with the column the query runs against, and lets the integration modules that build their databases at 8 or 64 dimensions run the production statement unchanged. The DDL side (027, both resets) still uses the configured dimension. 027 also reconciles an existing index of its name on a stamp-back: it accepts it only if it is valid and its `pg_get_indexdef` equals what `create_index_sql(D)` renders on a scratch TEMP table, and refuses anything else by name.
