...
- Read tools ship only what they render; results do not change beyond the ordering of exact ties.
...
- **Racy stats are not recorded.** If `max(mtime_ns, ctime_ns)` is within `STAT_RACY_WINDOW_NS` (2 s) of the wall-clock instant the hash completed, or in the future, the row's stat columns are written NULL, so the next pass hashes it (git's racy-clean rule). This closes the "same-size rewrite inside one timestamp tick after we read" case for local filesystems, whose timestamps come from the kernel's coarse clock.
...
| --- | --- | --- | --- |
| 1 | Same-size in-place rewrite within the recorded stat's timestamp granularity, after our read | Rare (editors and this server write by rename → new inode; appends change size) | Racy rule (D10) refuses to record such a stat on local filesystems; backstop otherwise |
...
| 5 | Subvolume/snapshot rollback of the vault root | Possible | Root identity (`st_dev`, `st_ino`, handle) changes → `classify_for_pass` re-derives → shortcut bypassed for every file |
...
The residual is L4: a second process running with different patterns could leave disagreeing rows. The next backstop sweep bounds this at 24 h.
...
- **L3** — **Stat-shortcut staleness.** An edit that changes none of `(size, mtime_ns, ctime_ns, inode)` is invisible to search until the next full-hash pass. That is at most `INDEX_FULL_HASH_INTERVAL_HOURS` (24 h) plus one pass, or until the file's stat next changes. Every known class is listed in D11. Network, FUSE and FAT mounts should set `INDEX_STAT_SHORTCUT=false`.
- **L4** — **Exclusion sweep skipped between backstops.** A second process running with different `EMBEDDING_EXCLUDE_PATTERNS` (a deploy overlap or a one-off container) could certify rows that disagree with this process's patterns without triggering a sweep here. Bounded by the 24 h backstop sweep.
...
- **L7** — **Deferred rows.** C5 defers a row changed mid-walk to the next pass. That note's row, and its search presence, can lag by one extra pass.
