...
**The clock advances only on success, so incomplete work cannot postpone the backstop** (Codex r1). `_last_full_hash[scope]` is set only when a full-hash scan pass for that scope **commits**. A full-hash pass that aborts, is refused (quarantine, generation mismatch) or is cancelled leaves the scope *due*, so every following pass for it stays a full-hash pass until one commits. A due backstop is never deferred by another interval.
