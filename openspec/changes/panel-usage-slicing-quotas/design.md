...
3. **Quota display on the keys page** (today's count / limit) reads the key's `quota_counters` row for the current UTC day (0 when absent — consumption is defined as admissions since the limit was enabled: the NULL-to-limited transition deletes the current-day counter row in its own transaction, while a limited-to-limited change keeps it; unlimited keys perform no quota accounting; the UI labels it so); usage page totals reuse the filter query.
