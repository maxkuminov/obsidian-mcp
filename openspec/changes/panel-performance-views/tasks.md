...
- [ ] 1.1 `indexer_runs` ORM model + migration 019; `make test-schema` green; `alembic check` clean after migrate

## 2. Instrumentation

- [ ] 2.1 Record a run row per pass (finally-block write; trigger source threaded from scheduled loop / panel reindex / backfill); prune to 500 in the same transaction

## 3. Panel

- [ ] 3.1 Performance page: per-tool percentiles + counts + response_size aggregates with 24h/7d/30d window selector (percentile_cont; created_at-bounded)
