...
- Add a "vs. hosted memory systems" comparison against the hosted/API-accessed memory architecture (mem0/Zep/Letta named as examples of the class; claims made at the architecture level, not as categorical assertions about named products) alongside the existing "vs. other Obsidian MCP servers" and "vs. an agent with raw file access" comparisons.
