...
**The provenance columns SHALL be able to record any value the facts they mirror can take.** That rule, not a width, is why the two pathname columns are `text`. A value the pass observed and cannot store is a bug, not a null: such a value SHALL NOT be truncated and SHALL NOT be recorded as null.
...
- **THEN** the write SHALL succeed and the stored value SHALL equal the observed path exactly
