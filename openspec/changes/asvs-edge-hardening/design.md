...
- **Timing.** A throttled attempt skips bcrypt and so answers faster than a wrong password against an active account. This is not newly introduced — an unknown username and an inactive account already skip it (`verify_password` runs only for an active row), so a timing difference between "no account" and "wrong password" exists in the tree today. Closing it is a separate change with its own cost, and it is listed as an accepted limitation rather than silently claimed away.
...
5. **Login response *timing* still distinguishes cases.** An unknown username, an inactive account and a throttled attempt all skip bcrypt; a wrong password against an active account does not. This predates the change and is not widened by it; equalising it is a separate change. D6a.
...
10. **The budget is per-worker.** It inherits the `--workers 1` contract like every other in-process control; a second worker would multiply the effective threshold by the worker count.
