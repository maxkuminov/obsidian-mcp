...
- **Lock ordering is fixed and stated: the advisory lock is acquired before any row or table lock** in any transaction that takes it. The reset takes it before it touches `note_embeddings`; the embed path takes it before `certify_embedded`'s conditional UPDATE takes its row lock. One direction everywhere, so the new lock cannot close a cycle with the row locks the pass and the panel already contend for.
...
| L5 | **A multi-tenant keyword rebuild is all-or-nothing, and a scope that cannot be rebuilt blocks it — and therefore, with FTS failing closed, blocks startup after an `FTS_CONFIGS` change.** Reachable by an unassigned user's leftover rows, a tenant still in re-derive mode, or ownerless rows under multi-user mode. | A fingerprint written over an unrebuilt scope certifies rows still on the previous configuration, which is the exact false claim the fingerprint exists to remove. | Settle the scope (assign or delete the user, let the re-derive finish), delete or reassign ownerless rows, **or revert `FTS_CONFIGS`**, which clears the refusal immediately with no rebuild. §8.7 checks for the reachable states before the deploy. |
...
- **Generation-lock takers (D7c/D7c2)** — the two certifying paths in the embed stage (`indexer.py:2622` backlog and `:2807` sweep, around `certify_embedded`, after the provider call); every writer of `content_tsvector` (`indexer.py:1820` incremental via `index_tsvector_sql`, and `_rebuild_tsvectors_pinned` at ~`:2907`); `scripts/reset_embeddings.py`; the panel reset paths; the rebuild driver. **Not** the exclusion branch (`indexer.py:2555`, `:2767`), which writes no vector — exemption argued in D7c.
...
14. **"The unverified ancillary passes do nothing for a user whose provenance is not settled"** (`index-integrity`) — the keyword rebuild stays gated per user; D7b's contribution is that a gated *skip* is now a typed outcome that aborts the driver instead of returning `0` and being read as success.
15. **The certification window is unchanged** (`index-integrity`, "after the embedding provider call … before any stored vector is deleted or inserted") — the advisory lock is acquired inside exactly that window, so no lock of any kind is held across a network request.
16. **Lock ordering is one direction everywhere** — the advisory generation lock before any row or table lock, in every transaction that takes it, so it cannot close a cycle with the `notes_metadata` row locks the pass, the panel and the discard branch already contend for.
