...
- **An enforced, per-response nonce policy on every HTML response rendered from the panel, auth and consent templates** (scope below); an existing enforcing policy on a response is never overwritten. Every `<script>` and `<style>` element in the panel, auth and consent templates carries the response's nonce, including the two vendored script tags and the pre-paint theme bootstrap, which stays inline. The policy, enumerated from the templates rather than guessed:
...
- **Scope is the panel surface, marked by the templates.** The policy applies to HTML rendered by the four panel/auth/consent `Jinja2Templates` instances — their shared context processor marks the request — including the HTML error renders (login 401, bootstrap register 400). It does not apply to the transfer pages (own instance, own stricter policy) or to FastAPI's `/docs`, `/redoc` and `/docs/oauth2-redirect`, which are framework HTML, not proxy-routed, and would break under a nonce-only policy. D7.
- **The consent page's `form-action` is `'self' https:`.** `POST /authorize` answers with a 302 to the registered `redirect_uri`, and Chromium applies `form-action` to every redirect hop after a form submission, so a flat `'self'` would break every OAuth connection. Owner decision after Codex round 1: an exact-origin source breaks multi-hop callbacks and non-canonical hosts, and a broken connector approve/deny is the most expensive failure available here, so the consent page admits any HTTPS target. The existing HTTPS-only `redirect_uri` registration rule and exact-match re-validation remain what decides where a code can go. D5.
- **`PANEL_CSP=enforce|report-only|off`**, default `enforce`, validated at boot (`Literal`), logged once at startup, with a warning when not enforcing. Rollback is one `.env` line and a container recreate — no rebuild. `report-only` sends the same policy as `Content-Security-Policy-Report-Only`; there is no report collection endpoint. **This deployment's rollout** starts in `report-only` and flips to `enforce` only after Max's browser pass, including a real connector approve and deny, shows zero violations.
- **Two gates in the test suite:** a header test through the real middleware stack — every HTML-declared route, plus explicit method/status cases (login 401, register 400, transfer `GET`/`HEAD`), since route metadata is not an exhaustive HTML inventory — asserting the header, a fresh nonce per response and every body nonce equal to it, and that `/docs` gets no panel policy; and a static scan of every template for `on*=` attributes, `javascript:` URLs, `hx-*` attributes, and `<script>`/`<style>` elements lacking the nonce.
...
- `panel-content-security-policy`: every HTML response rendered from the panel, auth and consent templates carries an enforced nonce-based Content-Security-Policy; the directive set is fixed and enumerated; the consent page's form-action is `'self' https:`; framework and transfer HTML are outside the surface; a boot-validated mode switch allows rollback; templates carry no inline handlers, no `javascript:` URLs and no un-nonced script or style elements; the eight existing confirm() controls fail closed; the panel loads no library that turns markup attributes into requests or code.
