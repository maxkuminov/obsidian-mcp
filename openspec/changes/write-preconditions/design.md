...
1. **Syntax.** If `expected_hash` is present and is not `sha256:<64 lowercase hex>` → `malformed_precondition`, **before any filesystem work at all**, in every tool and every mode. Malformed always wins: a caller who sent the wrong *kind* of value learns that, rather than learning something about a file that its argument never validly named.
...
6. **the in-call read** of the incumbent bytes through the pinned descriptor. `edit_note` and `set_frontmatter` already perform it; `write_file`, `move_note`, `delete_note` and `delete_file` perform it **only when a hash is supplied or required mode demands one**, so an unguarded call reads nothing it does not read today.
...
| `rewrite_links=True`, the moved note's own rewrite **failed** (the existing partial-success report) | the bytes the **rename** published — i.e. the unrewritten bytes now at `to_path` — never the intended rewritten bytes, which are not on disk |
| `rewrite_links=True`, a **backlink source's** rewrite failed | unaffected: the moved note's hash, per the two rows above |

Reporting an intended-but-unpublished hash would hand the agent a token that binds nothing and would make the next guarded write fail as `stale_precondition` against bytes that were never written. The two failure modes are tested separately, because they are different code paths with the same user-visible "partial success" wrapper.
