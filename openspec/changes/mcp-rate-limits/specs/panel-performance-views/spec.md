...
#### Scenario: Rate refusals are counted as refusals
- **WHEN** a window holds 40 executed `semantic_search` calls and 3000 rows carrying `error: "rate_limited"` for that tool
- **THEN** the percentiles are computed from the 40 executed rows only and the 3000 appear in that tool's refusal count

#### Scenario: An over-long-argument refusal is excluded from latency
- **WHEN** a window holds rows carrying `error: "argument_too_long"`
