...
**D7a. Planning retains ownership until acknowledgement.** An entry-local in-flight count pins each planned row's registered entry through the persistence await. A full, idle entry is still not evictable while it owns an unacknowledged row; new principals use the existing overflow entry at the cap. Cancellation restores the active row and every retired but unattempted row before propagating, so cancellation of the periodic indexer does not hide counts from the lifespan's final flush. A failed transfer claim release includes a conditional UPDATE returning false, not just an exception; either prevents the retryable 429.
