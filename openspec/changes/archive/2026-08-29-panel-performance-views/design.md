...
1. **Aggregates computed with `percentile_cont` over `usage_logs`** at request time, window-bounded (24h/7d/30d), grouped by tool, and filtered to executed rows via ONE shared predicate helper enumerating exactly the pre-body marker values (`over_quota: true`; the unencodable-argument marker; the `no_vault_assigned` marker — all written by `_tracked` before any body runs) — never a broad params-error match, which would wrongly exclude rows whose bodies ran and failed. The same helper feeds the per-tool refusal counts. The `created_at` index bounds the scan. No materialized rollups until proven slow — self-hosted volumes (hundreds of requests/day) don't justify them.
...
4. **Tables, not charts** (amended during implementation; originally "Chart.js reuse with theme-token colors"). Every figure the page reports is a small set of exact numbers an operator compares — six percentiles across ~25 tools, two phase means, a slowest list, a pass history. A bar chart of p95-per-tool is strictly less readable than the column it would replace and costs a canvas, a theme-token bridge for the chart palette, and a second rendering path to keep accessible. Charts earn their place on a *time series*, which this page does not have (the window selector picks a bucket, it does not plot one). If `panel-ops-health` (#163) adds pass duration over time, that is where Chart.js comes back. Colors still come only from `_theme.html` tokens per the `panel-theming` contract.
5. **Multi-tenant passes** record one row per pass (with the pass's `user_id` when per-user), matching how the indexer loops today. The page shows the newest 20 in a "Recent passes" card, with the owner **joined live** from `users` rather than denormalised: the FK is `ON DELETE SET NULL`, so when the user goes the row's claim about whose vault the pass indexed stops being true, and a stored label would keep asserting it. That is the opposite of `usage_logs`, whose `actor_*` columns exist precisely so a deleted credential still renders (#77) — the difference is that an actor is a historical fact about who called, and an owner is a live fact about who a row belongs to.
