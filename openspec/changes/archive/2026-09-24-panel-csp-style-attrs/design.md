...
- **Deploy (2026-09-24, beec182):** deployed first with `PANEL_CSP=report-only`. The in-container probe of `/admin/auth/login` returned `Content-Security-Policy-Report-Only` with `style-src-attr 'none'`, a nonced `style-src` fallback, no `'unsafe-inline'`, matching body nonces and no rendered `style=`. `/transfer/upload` keeps its own policy and `/docs` carries none.
- **Report-only walk (6.3):** the panel sits behind SSO, so the fallback target was chosen: the local instance of the same commit. Its enforce pass above covers that and is the stronger check, because enforce both blocks and reports and 0 violations were recorded. No separate report-only walk was run.
- **Enforce flip (6.4):** `PANEL_CSP=enforce` with a recreate and no rebuild. The startup log reads `PANEL_CSP = enforce`, and the login page carries the enforcing header with `style-src-attr 'none'` and no report-only header.
