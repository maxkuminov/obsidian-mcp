...
| 7 Integration | `src/services/security_events.py`, `tests/session_helpers.py` (new), every **existing** test module listed in §7 | 4 |
...
- [x] 1.4 `down_revision = "023"` — the sibling `index-integrity-hardening` migration. 024 MUST NOT merge or migrate ahead of it; if 023 is abandoned, rebase onto the then-current head and update the `schema-integrity` delta, which names the ordering.
- [x] 1.5 Confirm the model and the migration agree on every column, nullability, server default, index **name** and the table comment — `alembic check` compares index names and the comment, and does not compare server defaults at all, which is why the gate checks the catalog directly.
...
- [x] 2.4 `get_active_session_user`: require **both** `user_id` and `sid` — a cookie with no `sid` is a pre-deploy cookie and is **refused, not grandfathered**. Then, in order: resolve the row by `sha256(sid)`; refuse when absent, when `revoked_at` is set, when `expires_at <= now`, **when `row.user_id != cookie user_id`**; then the existing user-exists / `is_active` / `session_version` checks. Every refusal keeps calling `request.session.clear()`. On success, attempt the touch.
- [x] 2.5 `src/auth/routes.py`: `login_submit` mints through `start_session` **after** its existing `last_login_at` commit. `register_submit` mints **after** the bootstrap transaction has committed — never inside it, because that transaction holds `USER_BOOTSTRAP_LOCK_KEY` and must not be lengthened. The bootstrap key and the account guard are therefore taken **sequentially, never nested**, so no lock cycle is introduced. A mint that refuses (account gone or inactive) leaves the user signed out rather than raising.
- [x] 2.6 `src/auth/routes.py`: `logout` gains `session: AsyncSession = Depends(get_session)`, reads the `sid` **before** clearing, revokes that one row, commits, clears, redirects. On failure: attempt a rollback **inside its own guard** (a failing rollback must not escape either), record an ERROR carrying the exception's **class name only** — never `str(exc)` or `exc_info`, because SQLAlchemy renders bound parameters into the message and one of them is the stored session hash — then **still clear the cookie and still redirect**.
- [x] 2.7 `src/auth/routes.py`: `login_form` stops reading `request.session.get("user_id")` directly (`:150`) and resolves through `get_active_session_user`. Without this a revoked cookie bounces between the login page and `/admin/` forever.
- [x] 2.8 Emit `panel_session_replay_refused`, `panel_sessions_revoked` and the existing `panel_logout` through `src/services/security_events.py`. Slice 7 registers the new names in `EVENT_FIELDS`; this slice only calls them. **Never** the identifier, **its stored hash**, a password or a hash — use `token_tag`.
- [x] 2.9 `tests/test_issue_198_session_registry.py`: the regression the issue asks for — sign in, capture the cookie, log out, **replay it → 302 to login** — plus: the minted cookie authenticates the **very next** request and the row is visible from a separately opened session; a bootstrap mint commits outside the bootstrap lock's transaction; a cookie with no `sid` is refused; an expired row is refused while its signature is still valid; a revoked row is refused; a row whose `user_id` disagrees with the cookie is refused and cleared; a live row authenticates; a second session survives the first's logout; `login_form` renders the form for a revoked cookie; the touch is skipped inside the interval, skipped on `POST`, issued and committed on a stale `GET`, and takes **no second connection**; **more concurrent stale-session requests than `pool_size + max_overflow` complete without a pool timeout**; a raising touch still serves the page; a failing logout revocation *and* a failing rollback still clear and redirect; the failure record carries only a class name; no record contains the identifier, its stored hash, or any ≥12-character substring of either. **Mint races:** a deactivation committing between a login's password check and its mint leaves **no row** and no working cookie; the same during a post-change re-issue leaves no row and signs the browser out; and after a subsequent **reactivation** there is still no row from that window and no cookie from it authenticates.
- [x] 2.10 Confirm the new module **fails against the pre-change handlers** and passes after.

## 3. Revocation call sites, the password policy in user administration, and the purge

- [x] 3.1 `src/control_panel/users.py` `reset_password`: after the hash write and the `session_version` bump and **before** the existing `await session.commit()`, call `revoke_user_sessions(session, target.id)`. Add no commit — the account-guard lock is held and the guard's atomicity depends on that.
- [x] 3.2 `edit_user_submit`: when the write clears `is_active` for the target, revoke the target's sessions in the same transaction, before its commit. Only on the deactivating transition; a vault-path edit revokes nothing.
...
- [x] 4.1 `src/control_panel/routes.py`: `GET /admin/account` rendering `account.html` through `_panel_context(request, user, {"active": "account", ...})` — gated by the router-wide `require_user_panel`, **not** `require_admin_panel`. **404 in single-user mode, on the GET as well as the POST**: there is no account row and no local password there, so a page whose only content is a form that cannot exist is not a page, and the sidebar entry is already gated on `multi_user_mode`.
- [x] 4.2 `POST /admin/account/password` taking `current_password`, `new_password`, `new_password_confirm`; CSRF from the router-wide `verify_csrf`. **Two stacked limits**, both at `5/minute`: one `key_func=session_user_key`, one `key_func=get_remote_address`. Neither subsumes the other — an account-only key lets one address walk many accounts, and any key containing the address hands an attacker a fresh allowance per address, which is why the single composite key of the first draft did not bound guessing against one account. 404 when `settings.multi_user_mode` is false.
- [x] 4.3 **The critical section, in one transaction:** `lock_account_guard(session)`; re-read the acting user `SELECT … FOR UPDATE` with **`execution_options(populate_existing=True)`** — without it SQLAlchemy hands back the dependency-loaded object's **pre-lock** attribute values and the re-read proves nothing; refuse if the row is gone or `is_active` is not exactly true (the actor may have been deactivated while this request queued); verify `current_password` against the **freshly read** hash. Verifying against the dependency-loaded hash is how a concurrent admin reset gets silently overwritten and access the administrator just removed comes back.
- [x] 4.3a A request rejected by either limit never reaches the handler — slowapi's `RateLimitExceeded` is answered by the application-wide `_rate_limit_exceeded_handler` (`src/main.py:299`). **Leave that response alone**; it is deliberately exempt from the flash-and-303 rule. Do not add a second in-handler counter (it would diverge from the limiter that already decided) and do not replace the process-wide handler for one route.
- [x] 4.4 Refusals that *do* reach the handler, each `flash(...)` + `303` back to `/admin/account`, never a query parameter (#138): `validate_new_password` message; current password does not verify; the new password verifies against the stored hash (reuse); row gone or inactive. One constant message for the credential failure. Nothing commits between the lock and the write.
- [x] 4.5 Success, still in that transaction: `password_hash = hash_password(new)`, `session_version += 1`, `revoke_user_sessions(session, user.id)` — **all** rows, this session's included — then commit, which releases the lock. **After** the commit, `start_session(...)` runs in a second **guarded** transaction that re-takes the account guard and re-checks the account (2.1): the guard is released between the two, and an administrator can deactivate in exactly that gap. If the mint refuses or raises, the user is signed out; do **not** roll the password change back.
- [x] 4.6 `src/control_panel/templates/account.html` (new): username and role, the password form, and a line stating that changing the password signs the user's other browsers out. Existing classes and tokens only — **no colour literal**, or `colorscan` fails the sweep.
- [x] 4.7 `src/control_panel/templates/base.html`: an "Account" entry in the sidebar's **Access** section, gated on `multi_user_mode` like the Users link, with `{% if active == 'account' %}active{% endif %}`. SVG uses `stroke="currentColor"` like its neighbours — no literal.
- [x] 4.8 Emit `panel_password_changed` **after** the commit and `panel_password_change_refused` with the reason. No password, hash or identifier in any field.
- [x] 4.9 `tests/test_issue_197_password_change.py`: a change succeeds and the new password authenticates while the old one does not; **a concurrent admin reset committed before the lock is taken causes a refusal and is not overwritten, and no session is minted**; **a concurrent deactivation likewise**; a deleted row refuses; a wrong current password is refused and leaves the hash, `session_version` and every row untouched; mismatch, short, NUL and reuse each refused; the account-keyed limit bounds attempts across rotating addresses and the address-keyed limit bounds attempts across accounts, with successes counting against both; **the sixth attempt in the window answers with the application's rate-limit response rather than a flash and a redirect, and writes nothing**; a non-admin can reach the page; **both the GET and the POST 404 in single-user mode**; no CSRF token is a rejection; a second browser is dead afterwards while the changing browser is not; the pre-change cookie of the changing browser is dead too; a failing commit revokes nothing; a failing re-issue leaves the new password in force and the browser signed out; the redirect target carries no message parameter.
...
- [x] 5.4 `src/control_panel/templates/authorize.html`: an identity block above the request box — application name (as today), the **destination host** in its own labelled element, the client identifier, the registration date; the **unconditional** self-registration notice; then either the known-client badge or the warning naming the host and stating the code will be delivered there. The badge is worded about the destination — never "verified application". Nothing is `|safe`.
- [x] 5.5 Any new colour goes into `src/control_panel/templates/_theme.html` in **all three** blocks — bare `:root`, `:root[data-theme="light"]`, and the `prefers-color-scheme: light` copy — because `checks/token_coverage.py` asserts the two light copies are identical. Prefer the existing `--consent-warn`, `--consent-neutral-surface`, `--consent-text-2/3` and `--consent-surface-border` and add nothing.
- [x] 5.6 Do not touch the preselect: "Read only" stays `checked` unconditionally, the write radio never is, and the `autocomplete="off"` on the form and every radio stays. That is #63 and Firefox's checked-state restore.
- [x] 5.7 `tests/test_issue_183_consent_identity.py`: host, identifier and registration date render; `https://name@host/cb` displays the **host**; a non-ASCII host renders punycode, not decoded; registration refuses `https://@/cb` and a host that cannot be converted, and normalises one that can; a pre-existing hostless row renders "could not be determined" and takes the warning branch; an allow-listed host shows the badge and no warning; a non-allow-listed host shows the warning naming the host; prefix/suffix/substring look-alikes get the warning; an empty list warns for everything; the self-registration notice is present in **both** branches; a hostile `client_name` is escaped and does not reach the destination element.
- [x] 5.8 Run the light-mode sweep (`colorscan` / `literal_sweep.py` from the archived `2026-08-29-panel-light-mode` change) and confirm a **non-zero** template count as well as zero literals — a vacuous pass is #170.
...
- [x] 7.1 **Precondition:** the sibling's Slice A is merged and `src/services/security_events.py` exists. If it does not, this slice stops and the call sites fall back to a module logger under the same names (the substitution is mechanical).
- [x] 7.2 `src/services/security_events.py`: register `panel_session_replay_refused`, `panel_sessions_revoked`, `panel_password_changed` and `panel_password_change_refused` in `EVENT_FIELDS` with the fields named in the design's D19. Every field used is already in that change's `ALLOWED_FIELDS`; **add none**. `panel_logout` is already there.
...
- [x] 7.5 Update the existing modules that mint or assert a browser session, all of which construct a cookie by hand or drive `login_submit` and will fail once a `sid` and a row are required: `tests/test_auth_routes_real_hasher.py`, `tests/test_issue_4_authorize_post_redirect_validation.py`, `tests/test_security_review_followups.py`, `tests/test_followon_auth_routing.py`, `tests/test_issue_138_session_flash.py`, `tests/test_authorize_get_scope_preselect.py`, `tests/test_issue_21_registered_scope_enforced.py`, `tests/test_issue_64_ownerless_token_mint.py`, `tests/test_issue_67_offline_access_only_client.py`, `tests/test_issue_67_panel_scope_clamp.py`, `tests/test_issue_68_cross_user_client.py`, `tests/test_issue_92_oauth_error_headers.py`, `tests/integration/test_oauth_grants_pg.py`.
  - **Slice 3 repaired three of these, plus two the inventory misses.** The
    ...
    canned without one, so: `tests/test_security_review_followups.py` (its
    `db.execute.side_effect` list grew the fourth statement, and now
    asserts the revocation rides the same transaction),
    **`tests/test_issue_192_panel_forbidden.py`** and
    **`tests/test_users_vault_overlap.py`** — the last two are *not* in the
...
- [x] 7.7 Sweep for any remaining test that sets `session["user_id"]` without a `sid` and either fix it or assert it is a deliberate negative case.
  - Slice 2 repaired the **one** module the registry actually broke,
    ...
    assertions are still answered by the version check rather than by the new
    pre-registry refusal). Every other module in 7.5's inventory still passes on
...
- [x] 8.6 Confirm the `schema-integrity` MODIFIED block lands on the **existing** requirement at archive time rather than adding a second one of a similar name. (done 2026-09-21 at archive — `openspec/specs/schema-integrity/spec.md` holds exactly one "The schema gate covers both migrations of this wave before deploy"; this archive's MODIFIED block matched the text `index-integrity-hardening` had already applied, so it replaced the requirement in place with no diff and added no second similarly named requirement.)
