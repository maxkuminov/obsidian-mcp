...
- **Fence and heading lines end at LF, CRLF or a lone CR** (added under adversarial review). The read path normalizes before parsing, so only the raw-byte write paths could see the divergence — and they saw it as the block silently disappearing.
...
- `src/services/vault.py`: `parse_frontmatter_diagnose` beside `parse_frontmatter`, which itself gains two behavior changes. **One:** whitespace-only fenced YAML becomes a valid empty mapping for every consumer (read, index, diagnose), so `read_note` stops rendering such a block as body text. **Two (added under adversarial review):** a line ends at LF, CRLF **or a lone CR**, matching the universal-newline translation the read path applies before parsing — without it a classic-Mac note's block was stripped on read and diagnosed *absent* on write, so full-replace deleted it and `set_frontmatter` prepended a second one. Only raw-byte callers can observe the second change; the read and index paths normalize before parsing. The section helpers keep their signatures and their role — the body scoping lives in the `tools.py` caller, which holds a valid block out of the text it hands them and reattaches it afterwards — but `_scan_headings`, `_section_body_span` and `replace_section` take the same terminator rule, because a partition fix alone would turn section mode's safe "no ATX headings" refusal on a lone-CR note into a write against a bogus heading spanning the rest of the file
