...
- **Heading outline on truncation.** When a whole-note read is truncated, the response lists sections with their `#N` ordinal, depth, title, and size, flagging those that are themselves over the cap. The caller jumps straight to the section it wants instead of paging. The outline is **itself bounded by the cap** — it is appended to a response that exists because the content was too large, so an unbounded listing would recreate the problem. Overlong titles are elided; when the listing does not fit, it stops and reports how many sections were omitted plus the full ordinal range.
- **`#N` ordinal section selectors** (1-based, document order), accepted anywhere `section=` is — so `edit_note` gains them too. The existing `Parent/Child` form cannot disambiguate duplicate **sibling** headings, because they share every ancestor; generated notes repeat source filenames routinely. A bare `#N` is resolved **before** exact-text matching and always selects by position, so an ordinal we advertise in an outline can never be shadowed by note content. A heading literally titled `#2` stays reachable via the path-style form (`Parent/#2`, which never takes the ordinal branch) and via its own ordinal — under this ordering every section remains addressable, which is not true if text wins.
