...
- **A Unix-socket host is never TLS, and a socket can arrive by five routes.** asyncpg connects any host beginning with `/` with `create_unix_connection` and ignores `ssl` for it. The effective host list comes from, in order: the URL's netloc host (SQLAlchemy 2.0.52 does **not** percent-decode it; the validator decodes each raw host token itself, fail-closed, so `%2Fvar%2Frun%2Fpostgresql` is treated as a path); `?host=` query values (SQLAlchemy splits repeated keys and comma lists into a multi-host list, which may mix TCP and socket entries); a `service` / `servicefile` query key or `$PGSERVICE` / `$PGSERVICEFILE` (a service file can supply `host`); `$PGHOST`; and — when all of those are empty — asyncpg's **implicit default** `['/run/postgresql', '/var/run/postgresql', '/tmp', '/private/tmp', 'localhost']` on Linux. A strict mode reached through any of them connects in plaintext, and outside the lifespan (alembic, the maintenance scripts, the stdio entry point) nothing would notice. Codex round 1 finding 2; resolved by D3a/D3b.
...
- **Every production connection creator in the tree** (grep for `create_async_engine`, `create_engine`, `asyncpg.connect`, `asyncpg.create_pool`, `psql`, `pg_dump`):
  ...
  | --- | --- | --- |
  | `src/database.py` `engine` | the FastAPI app (lifespan, tools, panel, OAuth, indexer), `src/mcp_stdio.py`, `scripts/reset_embeddings.py`, `scripts/rebuild_tsvectors.py` (both import `engine`/`async_session` from `src.database`) | `connect_args` gains `**database_ssl_connect_args()`; the URL passed D3/D3a at settings construction; D3b listener under strict modes |
  | `alembic/env.py` `run_async_migrations` | `make db-migrate`, the deploy's `alembic upgrade head`, `make db-check` | the same helper, applied to the URL alembic actually resolves (`get_url()`), with the same D3/D3a refusal before the engine is built and the D3b listener under strict modes |
...
- **httpx's `trust_env=True` default reads the environment for every client** (`httpx/_config.py` `create_ssl_context`, and the proxy mounts): `HTTP_PROXY` / `HTTPS_PROXY` / `ALL_PROXY` / `NO_PROXY` choose a proxy per URL, `SSL_CERT_FILE` / `SSL_CERT_DIR` replace the trust anchor, and `.netrc` supplies credentials. With `verify=True` and none of those set, the trust anchor is **certifi's bundle** (`certifi.where()`), not the operating system's store. So an `http://127.0.0.1` Ollama URL plus an ambient `HTTP_PROXY` routes note text through a remote plaintext proxy, and a stray `SSL_CERT_FILE` silently changes who is trusted. Codex round 1 finding 1; resolved by D7.
- **`urllib.parse.urlsplit` and httpx do not parse the same strings the same way.** `urlsplit` strips leading C0/space characters and removes embedded tab/CR/LF; httpx does not. `' https://x/v1'` passes `urlsplit` as `https` but httpx sees no scheme (`UnsupportedProtocol`); a trailing newline passes `urlsplit` and raises `InvalidURL` in httpx. Codex round 1 finding 5; resolved by D6 rules 0 and 1.
...
- **A startup *warning* outside the lifespan.** `alembic/env.py`, `src/mcp_stdio.py` and the maintenance scripts get the same `connect_args`, the same pre-connect socket refusal (D3a) and the same per-connection check (D3b), so a strict mode is enforced on every connection they open; only the `prefer` *warning* is lifespan-only (accepted limitation 4).
- **An embedding proxy setting.** With `trust_env=False` an operator can no longer route embeddings through `HTTP(S)_PROXY`. Production has no such configuration; a proxy would be a later, explicit setting if anyone needs one (accepted limitation 9).
...
**D3a — Under a strict mode, every Unix-socket route is refused before any connection is attempted.**
`validate_database_url_transport(url, mode)` in `src/services/transport_security.py` is called from the settings validator (for `settings.database_url`) **and** at the top of `alembic/env.py`'s `run_async_migrations` (for the URL `get_url()` resolves). When the mode is `require`, `verify-ca` or `verify-full` it:
- builds the candidate host list from the parsed URL's netloc host **and** every `?host=` value (repeated keys and comma-separated lists both split, as SQLAlchemy does), percent-decoded;
- refuses an **empty** list, because asyncpg would then consult `$PGHOST` and then its implicit default, which starts with socket directories;
- refuses **any** candidate beginning with `/` (or `@`, the abstract-namespace spelling), so a mixed TCP/socket multi-host list is refused as well as an all-socket one;
- refuses a `service` or `servicefile` query key, and `$PGHOST`, `$PGHOSTADDR`, `$PGSERVICE` or `$PGSERVICEFILE` in the environment, because each can supply a host this check cannot see.
The message names the offending route and says a socket cannot carry TLS: use `disable`/`prefer` for a socket, or a TCP host for a strict mode. Under `disable`/`prefer` none of this applies; a socket is local and today's behaviour stays. Because the check runs at settings construction and before alembic builds its engine, no socket is opened under a strict mode. The tests prove it by asserting that neither `asyncpg.connect` nor `loop.create_unix_connection` is called (task 1.8a).

**D3b — Every new connection under a strict mode is verified, not only the first.**
Defence in depth behind D3a, and what closes the old "the probe checks one connection" limitation for strict modes. `install_strict_transport_listener(engine)` attaches a SQLAlchemy `connect` event to the engine's `sync_engine`. The event runs `SELECT ssl FROM pg_stat_ssl WHERE pid = pg_backend_pid()` on the raw DBAPI connection and raises when the result is not true, so the pool discards the connection. `src/database.py` and `alembic/env.py` install it under strict modes only. It costs one query per **new** pooled connection, never per checkout. A failure here, after D3a has passed, is a bug, so it raises instead of exiting and the caller sees an ordinary connection error.
...
0. **The raw string is checked before parsing.** Leading or trailing whitespace, or any C0 control character or DEL anywhere in it, → **refuse**. Nothing is normalised: the string the validator approves is byte-for-byte the string the providers interpolate (`f"{settings.ollama_url}/api/embed"`) and the report prints, so two parsers have nothing to disagree about.
1. Parse with **`httpx.URL`**, the parser the clients themselves use, so the host the policy judges is the host the request dials. An `httpx.InvalidURL`, a scheme not in `{http, https}`, an empty host, non-empty `userinfo`, or an invalid port → **refuse** (message names the setting, never echoes userinfo). `urlsplit` is not used anywhere in the policy. A differential test (task 2.5) runs every case through both parsers and asserts the policy follows httpx where they disagree.
2. `https` → accept.
3. `http` and `_is_loopback_host(url.host)` → accept. `url.host` is httpx's (IPv6 without brackets, lower-cased), so the host is what the client will actually dial: `http://127.0.0.1@evil.example` is refused by rule 1 and would have yielded `evil.example` anyway; `http://127.0.0.1.evil.example` is a DNS name, not an address, and is non-loopback; `http://[::1]:11434` and `http://localhost` are loopback. No DNS resolution — a name that happens to resolve to loopback is non-loopback, failing closed, as the sandbox guard already does.
...
5. **Sandbox rule.** `MCP_SANDBOX_MODE`, which never calls a provider, skips rules 0–4 and the CA-with-`http` refusal below — everything that depends on the endpoint URL. **It does not skip the CA file's own validity.** A configured `EMBEDDING_CA_FILE` that is missing or unparseable is a broken configuration in any mode, exactly like the `DATABASE_SSL_*` files, which are also checked in every mode.

`EMBEDDING_CA_FILE` (optional) is checked like `DATABASE_SSL_CA_FILE` (exists, regular file, readable) **and parsed at settings construction** into the `ssl.SSLContext` the clients will use (`ssl.create_default_context(cafile=…)`). An unparseable file therefore fails the boot with the setting named, whichever provider is active. The validated context is stored on the settings object (a `PrivateAttr`) and **reused**; nothing re-reads the file later. Round 0 parsed it lazily in the factory, which was wrong: the OpenAI provider makes no startup call, and Ollama's warm-up is asynchronous and swallows failures, so a bad CA would have booted "healthy" and then failed every embedding. The CA file is also **refused with an `http` URL**, because a trust anchor on a hop with no TLS verifies nothing and signals a misunderstanding.

*Why refuse by default rather than warn (the weighed alternative).* A warn-by-default would mirror `DATABASE_SSL_MODE=prefer` and break nothing. It was rejected because the two hops are not symmetric: `prefer` at least **attempts** TLS and becomes encrypted the day the server enables it, whereas an `http://` URL never will — it stays cleartext until someone edits the setting, and a warning in a log nobody reads is how #185 went unnoticed. An override makes the plaintext hop a **written decision in the operator's own `.env`**, which is ASVS V12.3.3's intent, costs one line, and is removed the day the hop gains TLS. The costs are real and accepted: (a) the default `OLLAMA_URL` no longer boots without the line, so `.env.example` ships `EMBEDDING_ALLOW_PLAINTEXT=true` directly beside the `http://ollama:11434` default with a comment explaining it; (b) an upgrading self-hoster with an `http://` sibling-container Ollama gets a boot refusal whose message states the one-line fix; (c) the test harness must set the override (tasks 5.1–5.3). **Owner-confirmed 2026-09-22** (see "Owner decisions").
...
**D7 — One client factory for every embedding HTTP client, with the environment switched off.**
`embedding_http_client(timeout: float) -> httpx.AsyncClient` in `src/services/transport_security.py` returns `httpx.AsyncClient(timeout=timeout, verify=<the validated CA context, or the cached certifi context>, follow_redirects=False, trust_env=False)`.
- **`trust_env=False`**, so no ambient variable can change the hop after the policy approved it: no `HTTP(S)_PROXY`/`ALL_PROXY` routing (an approved loopback `http` URL can no longer be carried to a remote proxy in cleartext), no `SSL_CERT_FILE`/`SSL_CERT_DIR` replacing the trust anchor, no `.netrc` credentials. The policy judges the URL, and with the environment off the URL is the whole hop.
- **The default trust anchor is certifi's bundle**, stated rather than implied. It is what `verify=True` already used, so the OpenAI provider's behaviour does not change. The factory builds that context itself, once and cached (`ssl.create_default_context(cafile=certifi.where())`), so the choice does not depend on `trust_env` and the report can name it: `verify=certifi`. With `EMBEDDING_CA_FILE` the anchor is that file only: `verify=ca-file`. The operating-system store is never consulted, and the report never says `system`.
- **`follow_redirects=False`** restates httpx's default explicitly, because a redirect is the one way a validated `https` endpoint could hand the request to an `http://` one after boot, and a default nobody wrote down is a default somebody changes.

The three sites in Context use it. A test asserts that no other `httpx.AsyncClient(` construction exists in `src/services/embeddings.py` or reaches `ollama_url` / `openai_base_url` in `src/control_panel/routes.py`, so a fourth site cannot bypass it silently.

**D8 — The startup transport report.**
After D4, the lifespan logs one INFO line per hop — the database line from D4, and `Embedding transport: provider=%s scheme=%s host=%s port=%s verify=%s plaintext_override=%s` where `verify` is `certifi`, `ca-file` or `n/a` (http). Host and port only; the URL's path, query and userinfo never reach the line. When the active embedding URL is plaintext and non-loopback (i.e. it was admitted by the override), the lifespan also emits `internal_transport_plaintext(reason="embedding", outcome="override")`. A plaintext **loopback** hop logs its line but emits no event — traffic that never leaves the network namespace is not what #185 is about. Under `MCP_SANDBOX_MODE` the report is skipped with the rest of the external checks.
...
**D10 — Module placement.** `src/services/transport_security.py` holds `database_ssl_connect_args(settings)`, `validate_database_url_transport(url, mode)` (the D3 URL-key refusal and the D3a socket refusal, one function shared by the validator and alembic), `install_strict_transport_listener(engine)` (D3b), `check_database_transport()`, `log_embedding_transport()` (the database line is logged inside `check_database_transport()`), `embedding_http_client()` and the pure `classify_embedding_url(url) -> (scheme, host, port, is_loopback)` (httpx-parsed, per D6 rules 0–1). `src/config.py` holds only the settings and the validators, which call pure helpers that do not import `src.database` (no cycle: `src.database` imports `src.config`). `alembic/env.py` already imports `src.config` transitively through `src.models.db`, so importing the helper there adds no new failure mode to the schema gate's subprocess.
...
3. **Under `prefer` the startup probe inspects one connection.** A later pooled connection could land differently, for example if the server's TLS setting changes while the app runs. Under strict modes every new connection is checked (D3b), so this limitation applies to `prefer` only.
4. **No `prefer` warning outside the FastAPI lifespan.** `alembic`, `src/mcp_stdio.py` and the maintenance scripts enforce strict modes (D2, D3a, D3b) but do not *warn* under `prefer`.
...
9. **No proxy for embeddings.** `trust_env=False` removes ambient proxy support on the embedding hop (Non-Goals). An operator who relied on `HTTP(S)_PROXY` for embeddings must drop it; none does in production.
...
1. **D6's default: refuse.** `EMBEDDING_ALLOW_PLAINTEXT` defaults to `false`; the owner adds the override line to the production deploy `.env` before deploying (task 7.1).
2. **Issue disposition:** #184 and #185 stay open until the infrastructure half lands. This change comments on them with what shipped and links the follow-up issue whose text is above.

## Spec review history

| Round | Finding | Severity | Resolution |
| --- | --- | --- | --- |
| 1 | httpx `trust_env=True` lets ambient proxies carry an approved loopback `http` hop off-host, lets `SSL_CERT_*` silently replace trust, and `verify=True` means certifi, not "system" | MAJOR | D7: `trust_env=False`; default anchor is certifi, built and named explicitly; report says `certifi`; proxy/CA-environment regression cases (task 2.6); limitation 9 |
| 1 | Strict modes can be bypassed by a Unix socket (URL host path, `?host=` incl. mixed multi-host, service files, `PGHOST`, the implicit default), and nothing outside the lifespan notices | MAJOR | D3a: pre-connect refusal of every socket route in the settings validator and in alembic; D3b: per-connection `pg_stat_ssl` check under strict modes; no-socket-attempt tests (task 1.8a); limitation 3 narrowed to `prefer` |
| 1 | `EMBEDDING_CA_FILE` parsed lazily, so an unparseable CA boots "healthy" with either provider; sandbox exception inconsistent | MAJOR | D6: parsed into an `SSLContext` at settings construction and reused; sandbox skips URL-dependent rules only, never file validity; startup tests for both providers (task 2.5) |
| 1 | Harness plan misses fresh `Settings(...)` constructors, generated dotenv fixtures, and offline lifespan fakes that would now hit the real database probe | MAJOR | Tasks 5.1–5.4 rewritten: autouse env fixture plus explicit subprocess/dotenv updates, probe stubbed in every offline lifespan test, dedicated tests opt out |
| 1 | `urlsplit` and httpx disagree on whitespace and control characters, so boot can approve a URL every request rejects | MINOR | D6 rules 0–1: raw-string check (no surrounding whitespace, no control characters), then parse with `httpx.URL`; differential-parser cases (task 2.5) |
| 2 | D3a judged raw URL tokens, not the hosts asyncpg actually receives: SQLAlchemy 2.0 turns `postgresql+asyncpg://u:p@db.example/app?host=:5432` into `host=''` (the query host wins over the netloc), which makes asyncpg fall back to `$PGHOST` and its socket directories although the netloc names a TCP host | MINOR | Folded in at implementation: `validate_database_url_transport` also computes the **effective** host list with the asyncpg dialect's own `create_connect_args` and, under a strict mode, refuses an empty list, an empty entry, a socket entry, or a URL SQLAlchemy cannot turn into arguments. `?host=:5432` cases with and without a TCP netloc are in task 1.8a's tests, for settings construction and for alembic's path, with sentinels proving no connection is attempted. Implementation also found SQLAlchemy 2.0.52 does **not** percent-decode the netloc host (the Context bullet above says it does); `%2Frun%2Fpostgresql` is refused anyway by decoding each raw candidate, fail-closed |

**Accepted limitation (verifier N1, 2026-09-22).** Under a strict mode the per-connection check (D3b) runs after SQLAlchemy's own dialect `connect`/`first_connect` handlers, so the dialect's setup queries (server version, codec setup) can run on a new connection before a plaintext one is discarded. No application statement runs on it, and D3a's pre-connect refusal makes the plaintext route unreachable in practice; reordering the listener ahead of dialect initialisation was judged riskier than the residual.

## Deploy and live check (2026-09-22)

- Deployed from main `536e8eb` (PR #284 app, PR #285 panel-csp). No migration; `alembic check`: "No new upgrade operations detected."
- Before the deploy, the deploy-dir `.env` gained `EMBEDDING_ALLOW_PLAINTEXT=true` (task 7.1); `DATABASE_SSL_MODE` left unset (`prefer`).
- Startup lines: `Database transport: mode=prefer encrypted=False tls_version=- server_verified=False` and `Embedding transport: provider=ollama scheme=http host=<redacted> port=11434 verify=n/a plaintext_override=True`, plus exactly two `internal_transport_plaintext` events (`reason=database outcome=prefer`; `reason=embedding outcome=override`). No ERROR/CRITICAL/Traceback lines in the first 15 minutes; the embedding pass ran for both users.
- End-to-end MCP check against the live server: `semantic_search`, `find_related` and `keyword_search` all returned correct, relevant results.
- Implementation review: `openspec-verifier` 0 blocking (N1 accepted above); adversarial Codex round 1 raised one MINOR, fixed; no round 2 under the review budget.
- Infrastructure half filed as #286. #184 and #185 stay open until it lands (owner decision 2).
