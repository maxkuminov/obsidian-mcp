...
- **The pass holds the generation lock for the duration of its transaction** — minutes on a large vault. `make reset-embeddings` and `make rebuild-tsvectors` therefore *wait* for an in-flight pass instead of interleaving with it, which is the behaviour we want and which the maintenance paths must not defeat with a short `lock_timeout`. **Nor with `statement_timeout`, which was the hole the first implementation left** (verifier pass 1): the engine sets it to 60 s per connection and the acquisition is a statement, so a maintenance command started against a live service was cancelled after a minute rather than waiting. Every waiting path — the two commands, both panel resets, and the incremental pass when *it* is the waiting side — lifts `statement_timeout` immediately before the acquisition and restores it immediately after. A `SET LOCAL` takes no row or table lock, so it does not put a lock ahead of the advisory lock and the ordering rule is untouched; the earlier reading, that the raise had to follow the acquisition, was wrong.
- **The embed path stays fine-grained, but discovery must end first.** A plain `SELECT` takes an `AccessShareLock` even without a row lock. The reconciliation enumeration reads `note_embeddings`; carrying that transaction through provider I/O makes reset's advisory-lock → `DROP INDEX` wait oppose certification's table-lock → advisory wait. The backlog and sweep enumerations and both per-note ORM lookups commit their read-only transactions before the provider call. Immutable result rows keep the verified hash/path snapshots, and `expire_on_commit=False` keeps the note attached for its later marker write. The after-provider transaction takes the generation lock first, re-reads the fingerprint, and retains conditional certification. Actual reset DDL, not only a fingerprint UPDATE, is required regression coverage.
