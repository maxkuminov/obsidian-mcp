...
2. **A static file under `/admin/static/`**, which would need `style-src-elem 'self'`. Rejected. `'self'` is wider than it looks on this origin. `/transfer/download` serves tenant-controlled vault bytes same-origin, with the MIME type from `mimetypes.guess_type`, so a `.css` note in any tenant's vault is served as `text/css`. `nosniff` does not help, because the type is honest, and `Content-Disposition: attachment` does not stop a subresource load. Under an HTML-injection bug, `<link rel=stylesheet href="/transfer/download/…">` would bring back exactly the stylesheet primitive (attribute-selector scraping of the CSRF token) that nonce-only `style-src-elem` exists to refuse. A path-scoped source (`https://<host>/admin/static/`) would need the hostname in the policy, and archived D5 forbids request-derived values in it. CSP also ignores source paths after a redirect.
...
Screenshots miss a 1 px padding change and flag every timestamp. So the same seeded instance is run twice, once on `f3ecba5` (before) and once on this branch (after), with the clock frozen (`page.clock`), animations settled (the count-up, progress and stagger durations are fixed in the templates; wait until `document.getAnimations()` is empty), and the same theme and viewport. Both runs use `PANEL_CSP=off`: the diff measures the cascade, not the policy, and the before-tree would otherwise differ only by what its own policy allows. For each page, theme (light, dark) and viewport (1280, 390), the diff:
...
- records `getComputedStyle` for a fixed property list: `display`, `position`, `box-sizing`, `margin-*`, `padding-*`, `border-*-width`, `border-*-style`, `border-*-color`, `border-radius`, `font-family`, `font-size`, `font-weight`, `line-height`, `letter-spacing`, `text-transform`, `text-align`, `white-space`, `color`, `background-color`, `background-image`, `opacity`, `transform`, `gap`, `flex-*`, `align-items`, `justify-content`, `grid-template-columns`, `width` and `height` (the last two only for elements that had an explicit width or height in their old inline style), `overflow`, `text-overflow`, `z-index`, `fill` and `stroke` (SVG);
- **passes only on zero differences**, apart from an explicit allow-list committed with the result (design.md "Verification record"), each entry a one-line reason. The expected allow-list is empty.
...
2. **First production deploy with `PANEL_CSP=report-only`** in the deploy directory's `.env`, then `make deploy`. In report-only mode the browser still reports every would-be violation in the console. So Max's walk with devtools is a real zero-violation check, not only a visual one. The fixed page list (tasks §6) covers each page in both themes plus the mobile width.
3. Zero reported violations → `PANEL_CSP=enforce` and a recreate (no rebuild). A short re-check: the login page header shows `style-src-attr 'none'`, and one panel page shows zero violations.
...
1. **Hover and focus states are checked by eye, not by the computed-style diff.** A converted element can now take a component `:hover`/`:focus` rule that its inline style used to mask. The known surfaces are hovered in D8. Anything else found on Max's walk is fixed forward.
...
- **Rollout (owner):** report-only first, then enforce, as #195 did and #289 asks for (the default above)? Or deploy straight to `enforce`, relying on the local enforce-mode pass and treating `report-only` as the rollback only? The default costs one extra recreate and one devtools walk.

## Verification record

*(Filled in during tasks §5–§6: the D9 diff counts and allow-list, the D8 result, the production walk.)*
