...
- **All panel CSS stays in nonced `<style>` elements.** A static stylesheet would need `style-src-elem 'self'`, and this origin serves tenant-controlled bytes with a guessed `text/css` type (`/transfer/download`), so `'self'` would bring back the CSS-injection primitive the nonce keeps out. D5.
...
- **Verification.** A headless Playwright pass runs against a local instance under `PANEL_CSP=enforce` with a seeded vault and seeded data. Every page must show zero CSP violations. Positive controls prove that an injected `style` attribute and an unnonced `<style>` are refused and reported, and that a CSSOM write still applies. A computed-style diff of every page, before and after the change, catches cascade regressions that a screenshot glance would miss. Production then goes `report-only` first and `enforce` after Max's walk with devtools, as in #195. D8, D9.
