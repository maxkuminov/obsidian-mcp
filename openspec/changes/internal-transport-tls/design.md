...
- **A Unix-socket host is never TLS, and a socket can arrive by five routes.** asyncpg connects any host beginning with `/` with `create_unix_connection` and ignores `ssl` for it. The effective host list comes from, in order: the URL's netloc host (SQLAlchemy percent-decodes, so `%2Fvar%2Frun%2Fpostgresql` is a path); `?host=` query values (SQLAlchemy splits repeated keys and comma lists into a multi-host list, which may mix TCP and socket entries); a `service` / `servicefile` query key or `$PGSERVICE` / `$PGSERVICEFILE` (a service file can supply `host`); `$PGHOST`; and — when all of those are empty — asyncpg's **implicit default** `['/run/postgresql', '/var/run/postgresql', '/tmp', '/private/tmp', 'localhost']` on Linux. A strict mode reached through any of them connects in plaintext, and outside the lifespan (alembic, the maintenance scripts, the stdio entry point) nothing would notice. Codex round 1 finding 2; resolved by D3a/D3b.
...
**D10 — Module placement.** `src/services/transport_security.py` holds `database_ssl_connect_args(settings)`, `validate_database_url_transport(url, mode)` (the D3 URL-key refusal and the D3a socket refusal, one function shared by the validator and alembic), `install_strict_transport_listener(engine)` (D3b), `check_database_transport()`, `log_transport_report()`, `embedding_http_client()` and the pure `classify_embedding_url(url) -> (scheme, host, port, is_loopback)` (httpx-parsed, per D6 rules 0–1). `src/config.py` holds only the settings and the validators, which call pure helpers that do not import `src.database` (no cycle: `src.database` imports `src.config`). `alembic/env.py` already imports `src.config` transitively through `src.models.db`, so importing the helper there adds no new failure mode to the schema gate's subprocess.
