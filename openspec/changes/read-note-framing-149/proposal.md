...
- **BREAKING:** `read_note` returns a **structured result** instead of a rendered string. The MCP tool declares an output schema and returns fields — no envelope to parse, so nothing note-controlled can forge a frame:
  ...
  - truncation as data: `truncated`, `offset`, `next_offset`, `total_chars`, and — whole-note reads only — the section `outline` as a list of `{ordinal, depth, text, size}` objects plus a human-readable `notice` string carrying the existing guidance prose.
