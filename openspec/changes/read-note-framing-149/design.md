...
**5. Every field gets a budget, and the worst case is restated.** `content` keeps the response-size cap; the outline keeps its independent budget; the metadata fields (`title`, `tags`, `frontmatter_yaml` + view, `heading`) share a new metadata budget (≤ cap) — on breach the oversized field is replaced by a bounded marker, because `read_note` goes through `read_file()` with no note-byte limit and a multi-megabyte frontmatter must not ride in beside a one-character body. The architecture doc's worst case is restated for the new shape: (content cap + outline budget + metadata budget + fixed prose) × 2 for the structured/text duplication × up to 6 for JSON escaping of note-controlled text. The old "≈2×cap" figure is retired with the envelope.
