...
| `panel_login_account_throttled` | WARNING | `client_ip`, `limit_count`, `route`, `username_submitted`, `window_seconds` | `src/auth/routes.py`, when the per-account failed-login budget refuses an attempt before the password is compared ([rate limits](rate-limits.md)). The subject is the **client address**, never `username_submitted` — a caller-supplied subject mints a fresh logging allowance per value rotated through it. No `user_id`: the record would then confirm that the submitted name resolves to an account, which the response deliberately does not. This is the only place the throttle is distinguishable — no response *content* differs from an ordinary failed login, though the timing does, because the comparison is skipped. `username_submitted` carries the same 64-character bound the other login events rely on; the event inherits it from the formatter and introduces no new one |
...
| `mcp_concurrency_pressure` | WARNING | `reason`, `outcome`, `limit_count`, `method`, `route`, `client_ip`, `user_id`, `key_id`, `oauth_token_id` | `APIKeyMiddleware`, once per transport admission (request envelope, auth permit) that saw pressure (#261, #188). `reason` is the closed `<stage>:<scope>` (e.g. `auth:global`, `request:fingerprint`). `outcome` is the closed set `MCP_CONCURRENCY_PRESSURE_OUTCOMES`: **`shadow`** a zero-wait would-refuse in shadow mode; **`refused`** an enforce refusal (deadline, waiter overflow) or a shutdown refusal — the caller got a transport 429; **`overrun`** queue mode admitted where enforce would have refused; **`waited`** a granted wait longer than 100 ms (`WAITED_EVENT_THRESHOLD_MS`). A shorter wait, and a waiter whose client disconnected, emit nothing: the ordinary cost of queueing is measured by the durable `concurrency_counters`, not this log, and a grant after a wait is never reported as a refusal. An unknown outcome is dropped, never emitted. No bearer fingerprint or token. Tool-stage and writer pressure is on the usage row, not here. Separate from the #263 body-outcome contract. |
...
| `usage_log_failed` | WARNING | `error_type`, `reason`, `tool` | `_log_usage` giving up; `reason` is `initial` or `after_clearing_fks`, or `concurrency_capacity` when the writer permit refused before any session opened (enforce writer refusal, or shutdown; counted as `writer_refused` in `concurrency_counters` unless shutdown) |
