...
- The leaf is re-`lstat`ed through the parent fd immediately before each tool acts, and a leaf that became a symlink is **named as one** — never reported as a missing note. "Not found" invites the agent to `create_note`, which would publish through the link.
...
- an **overwrite** (`edit_note`, `set_frontmatter`, `write_file(overwrite=True)`) is optimistic, not linearizable: `expected=` compares immediately before the rename and a writer landing inside that window is still overwritten — the same guarantee level as the transfer fingerprint check, declared rather than implied. The **no-clobber** publish (`create_note`, `write_file` by default) has no such window (`link()` is kernel-atomic), and neither does the soft delete or `move_note` (one `renameat2(RENAME_NOREPLACE)`).

Both are properties of concurrent editing, not of path resolution: the write always lands on the path the caller named, in the directory that was validated.
