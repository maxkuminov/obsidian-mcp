...
- **The no-clobber publish never exposes a staging name at all.** It stages into an `O_TMPFILE` inode — no directory entry, nothing to observe, replace or race — and publishes it with `linkat` through `/proc/self/fd/<fd>`. That also means there is nothing to clean up: a *named* staging file has to be unlinked, and an unlink is by name, so it can only be guarded by an identity check followed by the removal, which is check-then-act and could delete a substitute planted in between. `O_TMPFILE` removes the step rather than guarding it. **Do not add `O_EXCL`**: with `O_TMPFILE` it means "this file may never be linked into the filesystem", which makes the publish `ENOENT` — the opposite of its usual meaning. Two kernel details that look like blockers and are not: `linkat`'s `AT_EMPTY_PATH` form needs `CAP_DAC_READ_SEARCH` (an ordinary container has none) while the `/proc` magic link does not, and the "cannot link a zero-link inode" rule applies to an inode whose names were *removed*, not to one created `O_TMPFILE`. Verified on the deployment kernel at `CapEff=0`. No `O_TMPFILE`, or no `/proc`, means `UnsupportedFilesystem` — never a fall back to the by-name form.
- The **overwrite** publish is `renameat`, which is inherently by name, so its staging file does have one. `_require_staged_name` checks immediately before the rename that the name still refers to the inode we wrote, narrowing the substitution window to that one syscall; and `_discard_temp` unlinks it **only** while it still refers to our inode, otherwise leaving it in place and logging. The failure direction is to leave litter, never to remove something we cannot prove is ours — answering a substitution by deleting the substitute is the same destructive-write class, just aimed elsewhere.
...
- **an adversary who can write to the destination directory itself** can still win the `renameat` race on an overwrite publish. Say it plainly: that adversary can also just edit the note directly, so it is outside the threat #59 addresses — redirection through an *ancestor* or the *root*, where the attacker never had access to the destination at all. The no-clobber publish has no such window (nothing it stages is ever named).
- **the anchored walk is not atomic, and that one is inherited.** `vault_fs.open_dir_beneath` opens one component at a time, so an ancestor renamed *out of the vault* between two of those opens yields a parent descriptor outside the pinned root. The transfer routes and `delete_file` have always walked this way; #59 did not introduce it and does not close it. The fix is `openat2(RESOLVE_BENEATH | RESOLVE_NO_SYMLINKS | RESOLVE_NO_MAGICLINKS)` through `ctypes` — the kernel then enforces containment for the whole path in one call — which belongs to `vault_fs` and is its own change, tracked separately. Until then this is the last redirection window in the write path.
