...
**`soft_delete` is one `rename`, and it never unlinks anything.** A unique `<ts>-<basename>-<8 hex>` destination is reserved in `.trash` with `O_CREAT|O_EXCL|O_NOFOLLOW` and the source is renamed onto that placeholder. `link`+`unlink` was two syscalls and could unlink a *different* inode than the one it had copied — a writer landing in the middle lost its file with no trash copy. `rename` moves whichever inode is at the source when it runs, so a concurrent replacement ends up in the trash instead of being destroyed, and no verification step is needed to make that true. The `lstat` symlink/directory refusal still runs first and is deliberately not re-checked: a symlink swapped in afterwards is moved into `.trash` intact, never followed.
