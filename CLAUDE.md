...
- **The no-clobber publish carries the staged *inode*, not the staged name.** `linkat` through `/proc/self/fd/<fd>`, with the staging descriptor held open until publication. `link(tmp, name)` trusted `.tmp-…` to still mean what we wrote, and a peer in the destination directory could unlink or rename over it after the `fsync` and have *its* inode published as the note. Going through the descriptor takes the name out of the decision, and it fails closed: detaching our inode drops its link count to zero and `linkat` then returns `ENOENT` (only `CAP_DAC_READ_SEARCH` lifts that), so such a peer can prevent the write but never substitute it. Linux-only, which the declared filesystem semantics already require; no `/proc` means `UnsupportedFilesystem`, never a fall back to the by-name form. The **overwrite** publish is `renameat`, which is inherently by name, so it is preceded by an `fstat(fd)`-vs-`stat(tmp)` identity check that narrows the window to that one syscall. And `_discard_temp` unlinks the staging name only while it still refers to our inode — answering a substitution by deleting the substitute is the same destructive-write class, just aimed elsewhere.
...
- **`move_note` re-checks what actually moved.** `renameat2` relocates whichever inode is at the source when it runs — the property that keeps a replacement from being destroyed — so the regular-file check that ran before the preflight does not bind the commit. A directory or a symlink dropped at the source in between is what moves, and "Moved" would then key `notes_metadata` to a subtree or an alias. `_refuse_a_moved_non_file` `lstat`s the destination through its parent fd and rolls back with a second `RENAME_NOREPLACE` (as `soft_delete_at` does), never touching the database; if the rollback loses the race the error names where to recover it from. Unlike the soft delete, a **symlink** is refused too: a link is inert in `.trash` but not at a move destination.
...
All of these are properties of concurrent editing, or of an attacker who already holds the destination directory — not of path resolution: the write always lands on the path the caller named, in the directory that was validated.
